...
public:
    struct Node {
		...
		Node* prev = nullptr; // Adding previous pointer for convenience
...
private:
	...
	void BuildFastSearchList() {
		if (fast_search_list_size == 0 || head == nullptr) {
			return;
		}

		// Build fast search list for each node
		Node* current = head;
		for (int i = 0; i < list_size; i++) {
			Node* target = current;
			for (int k = 0; k < fast_search_list_size; k++) {
				// For the k-th entry, we need to find the 2^k successor
				int steps = (1 << k);
	...
	// Helper function to find the appropriate node for a given code
	Node* findNode(int code) {
		if (head == nullptr) {
			return nullptr;
		}

		// If code <= head's bound, store in head
		if (code <= head->bound) {
			return head;
		}
		...
		// Shouldn't reach here if b_prime is correctly set
		return head;
	}
...
public:
    ...
    void put(std::string str, T value) {
		...
		Node* targetNode = findNode(code);
		if (targetNode != nullptr) {
			targetNode->kv_map[str] = value;
		}
    }
	...
	T get(std::string str) {
		...
		Node* targetNode = findNode(code);
		if (targetNode != nullptr) {
			auto it = targetNode->kv_map.find(str);
			if (it != targetNode->kv_map.end()) {
				return it->second;
			}
		}
